...
2. **Merge the P3 PR** (`perf/p3-compiled-settings-plan`) if still open + green — via the `guy-lud` identity. Confirm the benchmark check passed (scan ≈ +4.7%, under the 10% gate). Then `git checkout master && git pull`.
